...
from apps.core.permissions import IsOwnerOrReadOnly, IsStaffOrReadOnly, IsOwnerOrIsStaff
...
class ProductDetailsViewSet(ModelViewSet):
    ...
    permission_classes = (IsStaffOrReadOnly, )
    staff_filterset_class = StaffProductDetailsFilter

    def get_serializer_class(self):
        if self.request.user.is_staff:
            return StaffProductDetailsSerializer
        else:
